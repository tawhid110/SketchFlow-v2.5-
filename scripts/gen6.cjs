...
export function useCanvasCore({ canvasObjects, interactionMode, brushType, brushSize, color, onSelectObjectForTransform, onSelectMultipleObjects, onContextMenuOpen, onStrokeEnd, mainCanvasRef, activeCanvasRef }: any) {
  ...
  const startDrawing = (e: any) => {
    ...
    if (interactionMode === 'transform') { if (hit) onSelectObjectForTransform?.(hit.id, e.shiftKey); else { setMarqueeStart({x,y}); setMarqueeEnd({x,y}); activeCanvasRef.current.setPointerCapture(e.pointerId); } return; }
    ...
    if (e.pointerType === 'touch') longPressTimer.current = setTimeout(() => { onContextMenuOpen?.(e.clientX, e.clientY, hit ? hit.id : null); isDrawing.current = false; }, 600);
    activeCanvasRef.current.setPointerCapture(e.pointerId); isDrawing.current = true; hasDragged.current = false; lastPos.current = {x,y}; nextPos.current = {x,y}; activeCanvasRef.current.getContext('2d')?.clearRect(0, 0, activeCanvasRef.current.width, activeCanvasRef.current.height);
  ...
  const draw = (e: any) => {
    ...
    if (longPressTimer.current && lastPos.current && Math.hypot(x - lastPos.current.x, y - lastPos.current.y) > 10) { clearTimeout(longPressTimer.current); longPressTimer.current = null; }
  ...
  return { pan, zoom, isSpacePressed, isPanning, marqueeStart, marqueeEnd, touchPoints, handleWheel, startDrawing, draw, stopDrawing, handleContextMenu, hTS, hTM, hTE };
}
`
};
